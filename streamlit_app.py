...
def page_settings():
    ...
    with st.container(border=True):
        ...
        with bc3:
            _imp_counter = st.session_state.get("_settings_imp_counter", 0)
            _import_file = st.file_uploader("📥 导入", type=["xlsx", "xls"],
                                            key=f"settings_import_{_imp_counter}",
                                            label_visibility="collapsed")
            if _import_file is not None:
                try:
                    _xls = pd.ExcelFile(_import_file)
                    _sheets = _xls.sheet_names
                    _progress = st.progress(0, text="正在导入...")
                    if "机台配置" in _sheets:
                        st.session_state.df_machine = pd.read_excel(_xls, sheet_name="机台配置", dtype=object).reindex(columns=COL_MACHINE).fillna("").astype(object)
                    _progress.progress(25, text="机台配置已导入")
                    if "注塑配置" in _sheets:
                        st.session_state.df_injection = pd.read_excel(_xls, sheet_name="注塑配置", dtype=object).reindex(columns=COL_INJECTION).fillna("").astype(object)
                    _progress.progress(50, text="注塑配置已导入")
                    if "包装配置" in _sheets:
                        st.session_state.df_pack = pd.read_excel(_xls, sheet_name="包装配置", dtype=object).reindex(columns=COL_PACK).fillna("").astype(object)
                    _progress.progress(75, text="包装配置已导入")
                    if "工单排产" in _sheets:
                        st.session_state.df_workorder = pd.read_excel(_xls, sheet_name="工单排产", dtype=object).reindex(columns=COL_WORKORDER).fillna("").astype(object)
                    _progress.progress(100, text="工单排产已导入")
                    save_all()
                    st.success(f"✅ 导入完成！机台{len(st.session_state.df_machine)} / 注塑{len(st.session_state.df_injection)} / 包装{len(st.session_state.df_pack)} / 工单{len(st.session_state.df_workorder)}")
                    st.session_state._settings_imp_counter = _imp_counter + 1
                    st.rerun()
                except Exception as e:
                    st.error(f"导入失败: {e}")
        with bc4:
